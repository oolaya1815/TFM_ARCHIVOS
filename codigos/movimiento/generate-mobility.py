...
def update_world(list, timeres, cont):
    global rangelist_moment
    global rangelist_global

    p = list.top  # el primer nodo
    #----------------------------------------------
    ...
    for i in range(0, list.nonodes):
        if i == 0:
            ...
            if p.state == MOVING:
                ...
                if distance(p.x,p.y, xf, yf) <= 15:  # realizamos un contador para ver
                    cont +=1
            if cont <= 1200:
                if p.state == WAITING:
                    if p.waittime <= timeres:                               # now done waiting
                        ...
                        p.xg, p.yg = random.uniform(p.xg, maxx), random.uniform(p.yg, maxy)          # generate new goal //cambio
                ...
                if p.state == WAITING:
                    if p.waittime <= timeres:                               # now done waiting
                        sp = generate_sleeporspeed(
                            minspeed, maxspeed)      # generate speed
                        if sp > p.maxspeed:
                            p.maxspeed = sp
                        p.state = MOVING                                    # change to moving
                        p.xg, p.yg = random.uniform(p.xg, (1+random.uniform(0, 1))*maxx), random.uniform(
                            p.yg, random.uniform(0,1)*maxy)          # generate new goal //cambio
                        p.xdot, p.ydot = calculate_speedvector(
                            p.x, p.y, p.xg, p.yg, sp)  # calc new speedvector
                    else:
                        p.waittime -= timeres            # little less waiting left
            ...
            if p.state == WAITING:
                if p.waittime <= timeres:                               # now done waiting
                    ...
                    if sp > p.maxspeed:
                        p.maxspeed = sp
                    p.state = MOVING                                    # change to moving
                    #p.xg, p.yg = generate_waypoint(p.x,maxx,p.y, maxy)          # generate new goal //cambio
                    p.xg, p.yg = random.uniform(p.xg, a.x), random.uniform(p.yg, a.y)          # generate new goal //cambio
                    p.xdot, p.ydot = calculate_speedvector(
                        p.x, p.y, p.xg, p.yg, sp)  # calc new speedvector
                else:
                    p.waittime -= timeres            # little less waiting left
...
def generate_waypoint(x, maxxx, y, maxxy):
    x = random.uniform(x, maxxx)
    y = random.uniform(y, maxxy)
    return x, y
...
def main():
    ...
    print("Author Marcus Lund�n, SICS")
    ...
    for i in range(0, noofnodes):
        ...
        '''xx, yy = generate_waypoint(0,0.1,0, 0.1)                  # position   posicion inicial que pienso que inicia pero no es asi
        xgg, ygg = generate_waypoint(0,0.1,0, 0.1)                # goal'''  #supuestamente debe iniciar en 0.1 y 0.2 pero no es asi
        xx, yy = random.uniform(maxx/100, maxx/10), random.uniform(maxy/100, maxy/10)                  # position
        xgg, ygg = random.uniform(maxx/2, maxx), random.uniform(maxy/2, maxy)                # goal
        xdo, ydo = calculate_speedvector(xx, yy, xgg, ygg, ss)  # speed
        '''if random.random() <= 0.5:                                     # state
            state = WAITING
            time = generate_sleeporspeed(minpause, maxpause)    # sleeptime   esto genera una probalidad de emepezar de nuevo
        else:                                                   #             es decir si se calcula una nueva posicion o se usa la que se
            state = MOVING
            time = 0'''
        state = WAITING
        time = generate_sleeporspeed(minpause, maxpause)
    ...
    for globalticks in range(0, timesteps):
        ...
        update_world(nodelist, resolution,cont)
        cont +=1
        ...
        p = nodelist.top
        timenow = (globalticks * resolution) - disregardtime # se valida que este todo en orden
        for i in range (0, noofnodes-1):
            ...
            for t in range(i+1, noofnodes):
                if q != None:
                    #check ranges
                    dd = distance(p.x, p.y, q.x, q.y) # calculo distancia nodo
        ...
        if globalticks >= starttime:  # hasta que el tiempo no se mayor al tiempo de incio no grabara
            realtime = (globalticks - starttime) * resolution   # punto
            update_log(nodelist, fil, realtime)   # ver este codigo
